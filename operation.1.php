...
if(isset($_POST['submit'])){
	...
	if($inputTask == "Bunkering Operation"){
			...
			for($day = 1; $day < 31; $day++){
				if($day != (int)$daystr){
					if($info[$day][$users] + $info[(int)$daystr][$users] <= $maxDuration){
						...
						echo $task_id;
						$result->free();
						$sql = "INSERT  INTO `task` (`idtask`, `name`, `duration`, `date`, `userid`) VALUES (NULL, '{$task_name}', '{$task_duration}', '{$newDate}', '{$user_id}')";
						if ($db->query($sql)) {
							// Redirect to homepage //
							//header("Location:  index.php");
							// Redirect to homepage //
						}	
						$sql = "DELETE FROM `task` WHERE `idtask` = " .$task_id;
						if ($db->query($sql)) {
							// Redirect to homepage //
							//header("Location:  index.php");
							// Redirect to homepage //
						...
						$priority = $inputTask;
						$sql = "INSERT  INTO `task` (`idtask`, `name`, `duration`, `date`, `userid`) VALUES (NULL, '{$priority}', 8, '{$inputDate}', '{$user_id}')";
						if ($db->query($sql)) {
							// Redirect to homepage //
							//header("Location:  index.php");
							// Redirect to homepage //
		...
		if($flag){
			...
			alert("'.$priority.' is added and due to limitation of the task duration, previous tasks are inserted into ' . $dates[2]. ", " . $dates[3]. ", " .$dates[4].' respectively");
			...
			<nav style="background-color:#800000;" class="navbar navbar-inverse navbar-fixed-top" role="navigation">
				<div class="container">
					...
					<div class="navbar-header">
						...
						<a class="navbar-brand" href="index.php">Furkan</a>
					</div>
					...
					<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
						<ul class="nav navbar-nav navbar-right">
                            ...
                            <li>
								<a href="operation.php"><i class="fa fa-cog"></i> Unexpected Task</a>
							</li>
							<li>
						...
						<form style action="<?=$_SERVER['PHP_SELF']?>" method="post">
							<div class="col-xs-12 col-sm-12 col-lg-12">
                                ...
                                <div class="form-group">
									<label for="inputTask" class="col-2 col-form-label">Task Description</label>
									<div class="col-10">
									<!-- This <select> and <option> is used for dropdown menu to select engineers-->
										<select class="form-control" id="inputTask" name="inputTask"> <?php
                                            echo "<option>Bunkering Operation</option>";
                                            echo "<option selected>Discharge Operation</option>";?>											 
										</select>
									<!-- This <select> and <option> is used for dropdown menu to select engineers-->
									</div>
								</div>
			...
			<!-- COPYRIGHT -->
				<footer>
					<div class="row text-center">
						<div class="col-lg-12">
							<p>Copyright &copy; <b>Furkan Sayin</b> 2017</p>
						</div>
					</div>
				</footer>
			<!-- COPYRIGHT -->
